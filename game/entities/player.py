...
def load_player_module():
    ...
    for name, anidata in data["animations"].items():
        size = anidata["width"], anidata["height"]
        # create the animation block
        data_block = animation.AnimationData(anidata["images"], size, anidata["time"], image_path)
...
class Player(entity.Entity):
    ...
    def update(self, dt):
        ...
        if player_animations[self.state].update_registry(self.aid, dt):
            self.image = player_animations[self.state].get_frame(self.aid)
            if self.xflip:
                self.image = filehandler.xflip(self.image)
        ...
        self.motion[1] = maths.lerp(self.motion[1], 0, 0.1)
        # yes
        self.moved = True
